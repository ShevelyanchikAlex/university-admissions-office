...
/**
 * Command go to edit application page.
 *
 * @author Alex Shevelyanchik
 * @version 1.0
 */
public class GoToEditApplicationPage implements Command {
    @Override
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        session.setAttribute(SessionAttribute.URL, SessionAttributeValue.CONTROLLER_COMMAND + CommandName.GO_TO_EDIT_APPLICATION_PAGE);

        RequestDispatcher requestDispatcher = request.getRequestDispatcher(PagePath.EDIT_APPLICATION_PAGE);
